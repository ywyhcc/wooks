...
+ (void)getUserInfoFromServer:(NSString *)userId complete:(void (^)(RCDUserInfo *))completeBlock {
    [RCDUserInfoAPI getUserInfo:userId anotherUserID:@""
...
+ (void)getOtherInfoFromServer:(NSString *)other complete:(void (^)(RCDUserInfo *userInfo))completeBlock{
    [RCDUserInfoAPI getUserInfo:[ProfileUtil getUserAccountID] anotherUserID:other
    complete:^(RCDUserInfo *userInfo) {
        if (!userInfo) {
            userInfo = [RCDDBManager getUser:other];
            if (!userInfo) {
                userInfo = [[RCDUserInfo alloc] init];
                userInfo.userId = other;
                userInfo.name = [self generateDefaultName:other];
            }
            userInfo = [self generateDefaultPortraitIfNeed:userInfo];
        } else {
            userInfo = [self generateDefaultPortraitIfNeed:userInfo];
...
//获取所有申请
+ (void)getApplyListFromServer:(void (^)(NSArray<RCDFriendInfo *> *))completeBlock {
    [RCDUserInfoAPI getApplyList:^(NSArray<RCDFriendInfo *> *friendList) {
        if (completeBlock) {
            completeBlock(friendList);
        }
    }];
...
+ (void)getAllApplyFriendListFromServer:(void (^)(NSArray<RCDFriendInfo *> *))completeBlock {
    [RCDUserInfoAPI getFriendList:^(NSArray<RCDFriendInfo *> *friendList) {
        [RCDDBManager clearFriends];
        [RCDDBManager saveFriends:friendList];
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
...
+ (void)acceptFriendRequest:(NSArray *)userIds complete:(void (^)(BOOL))completeBlock {
    [RCDUserInfoAPI acceptFriendRequest:userIds
                               complete:^(BOOL success) {
                                   if (success) {
                                       [self getFriendListFromServer:^(NSArray<RCDFriendInfo *> *friendList) {
                                           if (completeBlock) {
