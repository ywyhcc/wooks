...
- (void)deleteFriends {
    NSMutableArray *deleteFriendIDArr = [NSMutableArray arrayWithCapacity:0];
    for (RCDFriendInfo *info in self.allFriendArray) {
        if ([self.deleteIdArray containsObject:info.userId]) {
            [deleteFriendIDArr addObject:info.stAccount];
        }
    }
    [RCDUserInfoManager batchFriendDelete:[deleteFriendIDArr copy]
