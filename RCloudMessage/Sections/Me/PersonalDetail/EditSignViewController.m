...
- (void)updateCurrentUserInfo:(NSString *)name {
    [DEFAULTS setObject:name forKey:UserSingleSign];
...
- (void)saveUserName:(id)sender {
    if ([self checkUserName]) {
        ...
        NSDictionary *params = @{@"userInfoId":[ProfileUtil getUserProfile].userInfoID,@"comment":name};
...
- (BOOL)checkUserName {
    ...
    if (self.userNameTextField.text.length == 0) {
        errorMsg = @"不能为空";
    } else if (self.userNameTextField.text.length > 30) {
        errorMsg = @"签名不能超过30位";
    }
    BOOL lagel = YES;
...
- (void)textFieldDidChange:(UITextField *)textField {
    ...
    if (![toBeString isEqualToString:self.originNickName]) {
        [self.rightBtn buttonIsCanClick:YES buttonColor:[FPStyleGuide lightGrayColor] barButtonItem:self.rightBtn];
    } else {
        [self.rightBtn
            buttonIsCanClick:NO
