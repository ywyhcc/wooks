         ...
         complete:^(BOOL success) {
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                if (success == YES) {
                    ...
                    [cell.rightImageView sd_setImageWithURL:[NSURL URLWithString:urlStr]];
                    //在修改群组头像成功后，更新本地数据库。
...
- (void)addAllMembersToFriends{
    [RCDGroupAPI addAllFriendsToFriends:self.group.groupId complete:^(BOOL success) {
        if (success) {
            [self showAlert:@"请求发送成功"];
        } else {
            [self showAlert:@"请求发送失败"];
        }
    }];
}
