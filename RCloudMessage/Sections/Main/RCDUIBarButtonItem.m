...
//初始化不包含图片的UIBarButtonItem
- (RCDUIBarButtonItem *)initWithNewbuttonTitle:(NSString *)buttonTitle
                                 titleColor:(UIColor *)titleColor
                                buttonFrame:(CGRect)buttonFrame
                                     {
    self = [[RCDUIBarButtonItem alloc] init];
...
//    [self.button setTitle:buttonTitle forState:UIControlStateNormal];
//    [self.button setTitleColor:titleColor forState:UIControlStateNormal];
//     self.button.titleLabel.font = [UIFont boldSystemFontOfSize:11];
     UILabel *textLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 18, SCREEN_WIDTH / 4, self.button.height - 15)];
     textLabel.text = buttonTitle;
     textLabel.textColor = titleColor;
     textLabel.font = [UIFont boldSystemFontOfSize:15];
     [self.button addSubview:textLabel];
    self.customView = self.button;
    return self;
}
