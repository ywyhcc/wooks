//
//  NewLabelMemberViewController.m
//  SealTalk
//
...
@interface NewLabelMemberViewController ()<UICollectionViewDelegateFlowLayout, UICollectionViewDataSource,
...
@implementation NewLabelMemberViewController
...
- (void)initData {
    ...
    if (self.selectList.count > 0) {
        [self.selectUserList addObjectsFromArray:self.selectList];
    }
...
//    self.hud.color = [UIColor colorWithHexString:@"343637" alpha:0.5];
    ...
    if (self.isAllowsMultipleSelection == NO) {
        ...
        for (RCDFriendInfo *user in self.selectUserList) {
            [seletedUsersId addObject:user.userId];
        }
        
        if (self.callBack) {
            self.callBack(seletedUsersId);
        }
    }
