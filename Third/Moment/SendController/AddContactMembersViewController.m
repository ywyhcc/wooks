//
//  AddContactMembersViewController.m
//  SealTalk
//
...
@interface AddContactMembersViewController ()<UICollectionViewDelegateFlowLayout, UICollectionViewDataSource,
...
@implementation AddContactMembersViewController
...
- (void)initData {
    ...
    if (self.selectArr.count > 0) {
        [self.selectUserList addObjectsFromArray:self.selectArr];
    }
...
- (void)setRightButton {
    ...
    if (self.selectUserList.count > 0) {
        ...
        [self.rightBtn buttonIsCanClick:YES buttonColor:[FPStyleGuide weichatGreenColor] barButtonItem:self.rightBtn];
    } else {
        titleStr = RCDLocalizedString(@"confirm");

        [self.rightBtn
            buttonIsCanClick:NO
                 buttonColor:[RCDUtilities generateDynamicColor:[FPStyleGuide weichatGreenColor]
...
- (void)clickedDone:(id)sender {
    ...
    if (self.isAllowsMultipleSelection == NO) {
        ...
        if (self.rightCallBack) {
            self.rightCallBack(seletedUsersId);
        }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    //给控件填充数据
    [cell setModel:user hideRight:YES];
