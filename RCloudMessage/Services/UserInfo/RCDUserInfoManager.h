...
//获取所有未通过及已通过好友的请求信息
+ (void)getAllApplyFriendListFromServer:(void (^)(NSArray<RCDFriendInfo *> *friendList))completeBlock;

//获取所有好友申请
+ (void)getApplyListFromServer:(void (^)(NSArray<RCDFriendInfo *> *friendList))completeBlock;
...
+ (void)acceptFriendRequest:(NSArray *)userId complete:(void (^)(BOOL success))completeBlock;
