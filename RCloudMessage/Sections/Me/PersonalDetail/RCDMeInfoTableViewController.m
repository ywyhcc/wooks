...
- (void)uploadImageQuery:(NSString*)picURL{
    
    [self updateCurrentUserPortraitUri:picURL];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        ...
        case 2: {
            NSString *sealTalkNumber = [DEFAULTS stringForKey:WoosTalkID];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0) {
        if (indexPath.row == 0) {
            ...
            // 设置 SealTalk 号
            NSLog(@"%@",[DEFAULTS stringForKey:EditChangeWoosTalkID]);
            if ([[DEFAULTS stringForKey:EditChangeWoosTalkID] isEqualToString:@"0"] || [DEFAULTS stringForKey:EditChangeWoosTalkID].length == 0) {
        ...
        if (indexPath.row == 0) {
            ...
            MeMoreInfoTableViewController *nextVC = [[MeMoreInfoTableViewController alloc] init];
